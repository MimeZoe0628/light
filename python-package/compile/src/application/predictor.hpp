...
public:
  ...
  */
  Predictor(Boosting* boosting, int num_iteration,
	...
	/*
		根据不同的预测目标来定义预测过程。存在以下几种预测：
		1. 预测所在叶子索引：
		2. 预测贡献：
		3. 预测原始分数：
		4. 其他：
	*/
    if (is_predict_leaf_index) { // 是否预测所在叶子索引
      predict_fun_ = [this](const std::vector<std::pair<int, double>>& features, double* output) {
        int tid = omp_get_thread_num();						// tid：当前线程id
        if (num_feature_ > MAX_FEATURE && num_feature_ / static_cast<int>(features.size()) > SPARSITY) {// 特征数大于MAX_FEATURE且密集
    ...
    } else if (is_predict_contrib) { //	是否预测贡献
      predict_fun_ = [this](const std::vector<std::pair<int, double>>& features, double* output) {
  ...
  */
  void Predict(const char* data_filename, const char* result_filename, bool has_header) {
    ...
    (data_size_t, const std::vector<std::string>& lines) {
	  ...
	  /*
		1. 循环遍历解析文件得到的每一行（该循环进行了并行化优化）
		2. 将每一行数据解析到oneline_features（不包含label列以及0值列）向量中（特征索引，特征取值）
		3. 根据oneline_features进行预测，将预测结果保存到result中
		4. 将第i行结果保存到result_to_write[i]中
	  */
      OMP_INIT_EX();
