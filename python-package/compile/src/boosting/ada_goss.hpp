	...
	*/
	...
	public:
		...
		*/
		...
		~AdaGOSS() {
#ifdef TIMETAG
			Log::Info("AdaGOSS::subset costs %f", subset_time * 1e-3);
			Log::Info("AdaGOSS::re_init_tree costs %f", re_init_tree_time * 1e-3);
#endif
		}

		bool IsTerminateSampling(const int num_sample, const double bound) {
			bool result = num_sample >= bound || (num_sample == num_data_);
			Log::Warning("AdaGOSS terminate bound: %d >= %f, total: %d", num_sample, bound, num_data_);
			return result;
		}
		/*
		动态抽样算法终止条件边界
		...
		*/
		void ResetAdaGOSS() {
			...
			if (gbdt_config_->bagging_freq > 0 && gbdt_config_->bagging_fraction != 1.0f) {
				Log::Fatal("AdaGOSS 中不能使用bagging");
			}
		...
		*/
		data_size_t BaggingHelper(Random& cur_rand, data_size_t start, data_size_t cnt, data_size_t* buffer, data_size_t* buffer_right) {
			...
			double top_rate = log2(bound_) / num_data_; 
			double other_rate = (double)bound_ / num_data_;
			data_size_t top_k = static_cast<data_size_t>(ceil(cnt * top_rate));
			data_size_t other_k = static_cast<data_size_t>(cnt * other_rate);
