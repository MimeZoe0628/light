...
public:
  ...
  * \brief 根据重要性对特征进行抽样
  */
  virtual void FeatureSamplingByImportance(const std::vector<double> feature_importance) = 0;

  virtual void SetFeatureSamplingByImportance(const bool feature_sampling_by_importance) = 0;
  /*!
  * \brief 设置抽样得到的特征索引
  */
  virtual void SetFeatureSampledIndices(const std::vector<int> feature_sampled_indices) = 0;

  /*!
  * \brief 获取使用的特征数
  */
  virtual int UsedFeatureCount() = 0;

  /*!
  * \brief 获取有效特征的索引
  */
  virtual std::vector<int> GetValidFeatureIndices() = 0;

  /*!
  * \brief Using last trained tree to predict score then adding to out_score; 使用上一次训练的树来预测分数，然后将其添加到out_score中
