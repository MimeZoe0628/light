...
/*! \brief Store data for one histogram bin 存储一个直方图bin的数据 */
struct HistogramBinEntry {
public:
  /*! \brief Sum of gradients on this bin 这个bin中的一阶导数之和*/
  double sum_gradients = 0.0f;
  /*! \brief Sum of hessians on this bin 这个bin中的二阶导数之和*/
  double sum_hessians = 0.0f;
  /*! \brief Number of data on this bin 这个bin中的数据数量*/
  data_size_t cnt = 0;
  /*!
  * \brief Sum up (reducers) functions for histogram bin.	直方图bin的加和
  */
...
public:
  ...
  /*! \brief True if bin is trival (contains only one bin) 是否是平凡的（即只包含一个bin）*/
  ...
  */
  ...
  /*!
  * \brief Mapping bin into feature value 将bin映射为特征值
  * \param bin
  ...
  */
  ...
  /*!
  * \brief Construct feature value to bin mapper according feature values 根据特征的所有取值，将特征值构造到bin mapper
  * \param values (Sampled) values of this feature, Note: not include zero.	特征的值(Sampled),不包含0 
  * \param num_values number of values.			值的数量
  * \param total_sample_cnt number of total sample count, equal with values.size() + num_zeros		总的样本数，即值的数量+0值数量
  * \param max_bin The maximal number of bin					最大的bin的数量
  * \param min_data_in_bin min number of data in one bin		一个bin 中的最小数据量
  ...
  */
  ...
  /*!
  * \brief Deserilizing this object from buffer 从缓冲区中将数据反序列化
...
private:
  ...
  /*! \brief True if this feature is trival 当特征仅具有一个值时为Ture */
  bool is_trival_;
...
/*!
* \brief Interface for bin data. This class will store bin data for one feature.  为一个特征存储器bin数据，以原始顺序
...
*		对于密集数据而言，由于无需二次排序，所以会比OrderedBin快
*/
class Bin {
public:
  /*! \brief virtual destructor */
  ...
  *        Ordered_gradients[i] is aligned with data_indices[i]'s gradients (same for ordered_hessians).
			使用ordered_gradients和ordered_hessians来提高缓存命中率。朴素的方式是通过 gradients[data_indices[i]]来获取梯度。
