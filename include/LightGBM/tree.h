...
public:
  ...
  */
  ...
  /*!
  * \brief Performing a split on tree leaves.			执行一次树的叶子的划分
  * \param leaf Index of leaf to be split				将要划分的叶子索引
  * \param feature Index of feature; the converted index after removing useless features	在除去无用特征后的转换后的索引
  * \param real_feature Index of feature, the original index on data		特征索引，数据中原始的索引
  * \param threshold_bin Threshold(bin) of split				划分的bin的阈值（bin）
  * \param threshold_double Threshold on feature value			特征值的阈值
  * \param left_value Model Left child output					模型左孩子的输出
  * \param right_value Model Right child output					模型右孩子的输出
  * \param left_cnt Count of left child							左孩子的个数
  * \param right_cnt Count of right child						右孩子的个数
  * \param gain Split gain										划分增益
  * \param missing_type missing type							缺失类型
  * \param default_left default direction for missing value		缺失值的默认方向
  * \return The index of new leaf.								新叶子的索引
  */
  ...
  /*!
  * \brief Performing a split on tree leaves, with categorical feature				执行一次叶子划分，针对分类特征
  ...
  */
  ...
  /*!
  * \brief Adding prediction value of this tree model to scores			将当前树模型的预测值添加到分数
  ...
  * \param data The dataset
  * \param used_data_indices Indices of used data     所使用的数据的索引
  * \param num_data Number of total data
  * \param score Will add prediction to score
  */
  void AddPredictionToScore(const Dataset* data,
  ...
  /*!
  * \brief Prediction on one record				预测一条记录
  * \param feature_values Feature value of this record			所需预测的记录的特征值
  * \return Prediction result			返回预测结果
  */
  inline double Predict(const double* feature_values) const;
  inline double PredictByMap(const std::unordered_map<int, double>& feature_values) const;

  inline int PredictLeafIndex(const double* feature_values) const;			//预测的记录落在的叶子索引
...
private:
  ...
  /*!
  * \brief Find leaf index of which record belongs by features  找到当前记录所在叶子索引
