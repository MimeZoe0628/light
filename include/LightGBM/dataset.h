...
public:
  ...
  */
  ...
  /*!
  * \brief Initial with binary memory 从内存加载
  * \param memory Pointer to memory 内存指针
  */
...
private:
  ...
  /*! \brief Number of weights, used to check correct weight file 用来检查正确的权重文件*/
...
public:
  ...
  */
  ...
  /*! \brief 存储非零特征的计数 */
  //static std::vector<size_t> feature_non_zero_cnt_;
  /*! \brief 存储非零特征索引,该索引是valid_feature_indices_和feature_non_zero_cnt_的下标 */
  /*static std::vector<int> feature_non_zero_sorted_idx_;
  static long cnt_sum_;*/
private:
  const char* data_filename_;
  /*! \brief Store used features 存储使用的特征*/
  std::vector<std::unique_ptr<FeatureGroup>> feature_groups_;
  /*! \brief Mapper from real feature index to used index	真实特征索引到使用的特征索引的映射*/
  std::vector<int> used_feature_map_;
  /*! \brief Number of used features 使用的特征数*/
  int num_features_;
  /*! \brief Number of total features 总的特征数 */
