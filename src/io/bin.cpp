...
namespace LightGBM {
  ...
  /*!
	\brief 
	\param distinct_values		取值经过升序排序的distinct值
	\param counts				distinct_values对应的取值计数
	\param num_distinct_values	distinct值的个数
	\param max_bin				能够使用的最大的bin数
	\param total_cnt			distinct值的个数对应的各个值的计数总和
	\param min_data_in_bin		一个bin中需要具有的最小数据量
  */
  std::vector<double> GreedyFindBin(const double* distinct_values, const int* counts,
    ...
    CHECK(max_bin > 0);
	//如果特征的distinct取值个数 <= 最大bin数目
    if (num_distinct_values <= max_bin) {
      bin_upper_bound.clear();
      int cur_cnt_inbin = 0;				// 当前bin中的计数
      for (int i = 0; i < num_distinct_values - 1; ++i) {
        cur_cnt_inbin += counts[i];			// 累计当前bin中的计数
		// 如果当前bin中的计数已经>= bin中所需最小的数据量，计算bin中特征取值的上界，并重置当前bin中计数
    ...
    } else {	//如果特征的distinct取值个数 > 最大bin数目
	  ...
	  // 存放discount计数是否大于mean_bin_size的标记
	  ...
	  // 利用（剩下的小计数的特征值的样本数）/（剩余bin数）重新计算 bin中存放的平均数据大小
      ...
      int bin_cnt = 0;
	  //第一个bin的下界设置为当前distinct的最小值
      lower_bounds[bin_cnt] = distinct_values[0];
      int cur_cnt_inbin = 0;
      for (int i = 0; i < num_distinct_values - 1; ++i) {
        if (!is_big_count_value[i]) {	//非大计数的特征值
          rest_sample_cnt -= counts[i];
        }
        cur_cnt_inbin += counts[i];
        // need a new bin：当为大计数值 || 当前bin中的数据量>bin中平均数据量大小 || 下一个特征值为大计数值且当前bin中的数量>=bin中平均数据量的一半
        if (is_big_count_value[i] || cur_cnt_inbin >= mean_bin_size ||
          ...
          cur_cnt_inbin = 0;
          if (!is_big_count_value[i]) {		//小计数特征值
            --rest_bin_cnt;
      ...
      ++bin_cnt;
      // update bin upper bound		更新bin的上界
      bin_upper_bound.clear();
  ...
  /*
	\brief 
	\param distinct_values		一列的distinct值 
	\param counts				distinct值对应的计数
	\param num_distinct_values	一列中distinct值的大小
	\param max_bin				一个bin中最大数据量
	\param total_sample_cnt		总的样本大小（输入数据的行数）
	\param min_data_in_bin		一个bin中所需的最小的数据量
	\return 
  */
  std::vector<double> FindBinWithZeroAsOneBin(const double* distinct_values, const int* counts,
    ...
    for (int i = 0; i < num_distinct_values; ++i) {
      ...
      } else{		//[-kZeroAsMissingValueRange, kZeroAsMissingValueRange]范围内的值视为0
        cnt_zero += counts[i];
      }
    }
    ...
    if (left_cnt > 0) {
		// 小于0的特征distinct 取值的计数占所有非0值特征distinct取值的比例，根据它来分配bin的数量
  ...
  void BinMapper::FindBin(double* values, int num_sample_values, size_t total_sample_cnt,
    ...
    // find distinct_values first	对一列的特征的各个取值进行保存并计数
    ...
    for (int i = 1; i < num_sample_values; ++i) {
      ...
      if (!Common::CheckDoubleEqualOrdered(values[i - 1], values[i])) {
        ...
        counts.push_back(1);
      } else {			//如果两个取值相等（足够相等视为相等）
        // use the large value	取值保存大的那个，计数++
    ...
    if (bin_type_ == BinType::NumericalBin) {
      ...
      {
		 // 统计满足bin_upper_bound_中上界的distinct值的计数之和
