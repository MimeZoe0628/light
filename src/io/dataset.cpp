...
/*!
	\brief  获取冲突数（即特征同时取非零值的数量）
	\param mark 表示样本是否被使用的标记
	\param indices	特定特征值对应的样本索引
	\param num_indices	样本索引大小
	\param max_cnt	最大冲突数
	\return 返回冲突数，当大于预置的冲突数，表示特征不互斥，返回-1
*/
...
std::vector<std::vector<int>> FindGroups(const std::vector<std::unique_ptr<BinMapper>>& bin_mappers,
  ...
  std::vector<std::vector<int>> features_in_group;				//group中存放互斥特征
  std::vector<std::vector<bool>> conflict_marks;				//冲突标记
  std::vector<int> group_conflict_cnt;							//group中发生冲突的样本数
  std::vector<size_t> group_non_zero_cnt;						
  std::vector<int> group_num_bin;
  
  for (auto fidx : find_order) {
    const size_t cur_non_zero_cnt = num_per_col[fidx];		//获取当前非零特征值的样本计数
    bool need_new_group = true;
    std::vector<int> available_groups;						//可用的组
	// 遍历group中的元素
...
/*!
	\brief 互斥的特征（不同时取非零值的特征）捆绑到一个特征
	\param bin_mapper 
	\param sample_indices 各个特征值对应的样本索引
	\param total_sample_cnt 总的样本数
	\param used_features 使用的特征对应的索引
	\param max_conflict_rate 容许的最大冲突率
	\param num_data 总的数据量
	\param min_data 叶子上的最小数据量
	\param sparse_threshold 
	\param is_enable_sparse 
	\param is_use_gpu
	\return 对互斥特征进行bundling的结果
*/
std::vector<std::vector<int>> FastFeatureBundling(std::vector<std::unique_ptr<BinMapper>>& bin_mappers,
                                                  ...
                                                  bool is_use_gpu) {
  // filter is based on sampling data, so decrease its range.   filter基于样本数据，以便减少其范围？
  const data_size_t filter_cnt = static_cast<data_size_t>(static_cast<double>(0.95 * min_data) / num_data * total_sample_cnt);
  const data_size_t max_error_cnt = static_cast<data_size_t>(total_sample_cnt * max_conflict_rate);	//能够容许的最大冲突数
  ...
  // sort by non zero cnt, bigger first 根据非0特征值的计数降序排序
  ...
  // add by zoe
  /*
	  Dataset::feature_non_zero_cnt_ = feature_non_zero_cnt;
	  Dataset::feature_non_zero_sorted_idx_ = sorted_idx;
	  Dataset::cnt_sum_ = cnt_sum;
  */
  /*
	根据特征互斥性进行两次group构造：
	1、根据当前使用的特征构造group
	2、根据当前使用的根据非零值计数进行排序的特征构造group（可能构造出优于1、的group）
  */
...
void Dataset::Construct(
  ...
  real_feature_idx_.resize(num_features_);			// 初始化真正使用的特征索引
  ...
  for (int i = 0; i < num_groups_; ++i) {
    ...
    for (int j = 0; j < cur_cnt_features; ++j) {
      int real_fidx = cur_features[j];					//真实的特征索引
      used_feature_map_[real_fidx] = cur_fidx;			//真实特征索引到当前特征索引
      real_feature_idx_[cur_fidx] = real_fidx;			//获取真实的特征索引
      feature2group_[cur_fidx] = i;						//根据当前索引获取特征所在group索引
      feature2subfeature_[cur_fidx] = j;				//根据当前索引获取特征在group中特征向量中的索引
...
void Dataset::ConstructHistograms(const std::vector<int8_t>& is_feature_used,
  ...
  for (int group = 0; group < num_groups_; ++group) {
	...
	//遍历group中的特征，如果group中特征使用过，则当前group已经存在于used_group中
