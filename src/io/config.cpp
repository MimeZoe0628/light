...
void GetBoostingType(const std::unordered_map<std::string, std::string>& params, std::string* boosting_type) {
  ...
  if (ConfigBase::GetString(params, "boosting_type", &value)) {
    ...
    if (value == std::string("gbdt") || value == std::string("gbrt")) {
      ...
      *boosting_type = "rf";
    }else if (value == std::string("gbdt_sampling")) {
		*boosting_type = "gbdt_sampling";
	}else if (value == std::string("ada_goss")) {
		*boosting_type = "ada_goss";
	}else {
...
void BoostingConfig::Set(const std::unordered_map<std::string, std::string>& params) {
  ...
  //动态抽样配置获取
  GetDouble(params, "error", &error);
  GetDouble(params, "confidence", &confidence);
  //特征抽样参数配置获取
  GetString(params, "sub_feature_method", &sub_feature_method);
}
