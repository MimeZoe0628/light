...
public:
  ...
  */
  ...
  /*!
  * \brief Perform a full training procedure		执行整个训练过程
  * \param snapshot_freq frequence of snapshot		snapshot频率
  * \param model_output_path path of model file		模型文件输出路径
  */
  void Train(int snapshot_freq, const std::string& model_output_path) override;

  /*!
  * \brief Training logic			训练逻辑
  * \param gradients nullptr for using default objective, otherwise use self-defined boosting		使用默认目标函数时为null，否则使用自定义的boosting
  * \param hessians nullptr for using default objective, otherwise use self-defined boosting
  * \return True if cannot train any more			如果无需再训练返回True
  */
  virtual bool TrainOneIter(const score_t* gradients, const score_t* hessians) override;

  /*!
  * \brief Rollback one iteration			回滚一个迭代
  */
  void RollbackOneIter() override;

  /*!
  * \brief Get current iteration		获取当前迭代数 ： 当前模型数/每一步迭代的树数
  */
  ...
  */
  ...
  /*!
  * \brief Get evaluation result at data_idx data				获取指定数据上的估计结果
  * \param data_idx 0: training data, 1: 1st validation data	0：训练数据 1：验证数据
  * \return evaluation result			估计结果（double类型的向量）
  */
  ...
  */
  ...
  void PredictCurIter(const double* features, double* output, const int curIter) const;
  ...
  */
  ...
  std::vector<double> NewFeatureImportance(int current_iteration, int importance_type) const override;
...
protected:
  ...
  */
  ...
  /*!
  * \brief Helper function for bagging, used for multi-threading optimization  用来对bagging进行过线程优化
  ...
  */
  ...
  /*!
  * \brief calculate the object function			计算目标函数
  */
  virtual void Boosting();

  /*!
  * \brief updating score after tree was trained			训练完树之后更新分数
  ...
  */
  ...
  /*!
  * \brief Print metric result of current iteration	打印当前迭代的度量结果
  * \param iter Current interation
  * \return best_msg if met early_stopping			当训练过程提前终止时的消息
  */
  ...
  /*! \brief First order derivative of training data 训练数据的一阶导数*/
  std::vector<score_t> gradients_;
  /*! \brief Secend order derivative of training data 训练数据的二阶导数*/
