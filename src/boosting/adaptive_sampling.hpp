...
namespace LightGBM {
#ifdef TIMETAG
	extern std::chrono::duration<double, std::milli> subset_time;
	extern std::chrono::duration<double, std::milli> re_init_tree_time;
	extern std::chrono::duration<double, std::milli> out_of_bag_score_time;
	extern std::chrono::duration<double, std::milli> valid_score_time;
	extern std::chrono::duration<double, std::milli> metric_time;
	extern std::chrono::duration<double, std::milli> bagging_time;
	extern std::chrono::duration<double, std::milli> tree_time;
	extern std::chrono::duration<double, std::milli> boosting_time;
	extern std::chrono::duration<double, std::milli> gbdt_train_time;
	std::chrono::duration<double, std::milli> cal_scores_time;
#endif // TIMETAG
...
public:
	...
	*/
	...
	~AdaptiveSampling() {
		...
		Log::Info("AdaptiveSampling::scores costs %f", cal_scores_time * 1e-3);*/
		Log::Info("AdaptiveSampling::sample size %f", sample_size_ * 1e-3);
#endif
	}
	...
	*/
	data_size_t GetBound(const double error, const double confidence, const double prob) {
		...
		//double new_error = error*std::abs(prob - 0.5) / (1 + error);
...
//#endif
			...
			for (auto& sub_metric : training_metrics_) {
				...
				for (size_t k = 0; k < name.size(); ++k) {
					if (name[k] == std::string("binary_error")) { 
