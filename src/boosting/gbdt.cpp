...
std::chrono::duration<double, std::milli> ucb_time;
std::chrono::duration<double, std::milli> gbdt_train_time;
#endif // TIMETAG
...
GBDT::~GBDT() {
  ...
  Log::Info("GBDT::ucb costs %f", ucb_time * 1e-3);
  Log::Info("GBDT::train costs %f", gbdt_train_time * 1e-3);
  #endif
}
/*
	GBDT初始化：
	...
	2、初始化配置：
		iter_:当前迭代数
		num_iteration_for_pred_:使用的模型数
		max_feature_idx_:训练数据的最大特征索引
		num_class：类标数目
		gbdt_config_:
		early_stopping_round_:提前终止时的循环数
		shrinkage_rate_:学习率
...
void GBDT::Init(const BoostingConfig* config, const Dataset* train_data, const ObjectiveFunction* objective_function,
  ...
  // create buffer for gradients and hessians		创建梯度缓冲区
  if (objective_function_ != nullptr) {
    size_t total_size = static_cast<size_t>(num_data_) * num_tree_per_iteration_;
    gradients_.resize(total_size);
    hessians_.resize(total_size);
  }
  // get max feature index							获取最大特征索引（总的特征数-1）
  max_feature_idx_ = train_data_->num_total_features() - 1;
  // get label index								获取类标索引
  label_idx_ = train_data_->label_idx();
  // get feature names				获取特征名称
  feature_names_ = train_data_->feature_names();
  feature_infos_ = train_data_->feature_infos();

  // if need bagging, create buffer					如果需要进行bagging，创建相应缓冲区
  ...
  //初始化特征选择对象
  feature_selection_ = new FeatureSelection();
  feature_selection_->Init(gbdt_config_->tree_config.feature_fraction, gbdt_config_->tree_config.feature_fraction_seed);
}
...
void GBDT::Boosting() {
  ...
  // objective function will calculate gradients and hessians. 目标函数将会计算一阶导数和二阶导数
  int64_t num_score = 0;
   objective_function_->
...
*
* An improvement to this is to have options to explicitly choose. 该实现存在如下显示可选项
* (i) standard average					标准average
* (ii) custom average if available		自定义average（如果存在的的话）
...
double ObtainAutomaticInitialScore(const ObjectiveFunction* fobj, const float* label, data_size_t num_data) {
  double init_score = 0.0f;				//对最初的分数进行初始化
...
bool GBDT::TrainOneIter(const score_t* gradients, const score_t* hessians) {
  ...
  for (int cur_tree_id = 0; cur_tree_id < num_tree_per_iteration_; ++cur_tree_id) {
    ...
    if (class_need_train_[cur_tree_id]) {
	  ...
	  //梯度数组大小与原始数据大小一致还是与bagging后的数据大小一致???
	  ...
	  if (gbdt_config_->sub_feature_method == "split") {
		  if (iter_ != 0) {
			  std::vector<double> feature_importances = NewFeatureImportance(iter_ + 1, 0);
			  tree_learner_->FeatureSamplingByImportance(feature_importances);
		  }
	  }
	  else if (gbdt_config_->sub_feature_method == "gain") {
		  if (iter_ != 0) {
			  std::vector<double> feature_importances = NewFeatureImportance(iter_ + 1, 1);
			  tree_learner_->FeatureSamplingByImportance(feature_importances);
		  }
	  }
	else if (gbdt_config_->sub_feature_method == "ucb") {
#ifdef TIMETAG
		auto ucb_start_time = std::chrono::steady_clock::now();
#endif
		if (iter_ == 0) {
			tree_learner_->SetFeatureSampledIndices(feature_selection_->RandomSelection(tree_learner_->GetValidFeatureIndices()));
		}
		else {
			std::vector<double> gain_importance = NewFeatureImportance(iter_ + 1, 1);
			std::vector<int> valid_feature_indices = tree_learner_->GetValidFeatureIndices();
			//tree_learner_->SetFeatureSampledIndices(feature_selection_->StratifiedSampling(&gain_importance, &valid_feature_indices, train_data_));
			tree_learner_->SetFeatureSampledIndices(feature_selection_->NaiveUCB(&gain_importance, &valid_feature_indices));
			//tree_learner_->SetFeatureSampledIndices(feature_selection_->RandomNaiveUCB(&gain_importance, &valid_feature_indices));

		}// end if
#ifdef TIMETAG
		ucb_time += std::chrono::steady_clock::now() - ucb_start_time;
#endif
	}
	else {
    ...
    if (new_tree->num_leaves() > 1) {
      ...
      // only add default score one-time 今天加默认分数一次
...
#ifdef TIMETAG
  gbdt_train_time += std::chrono::steady_clock::now() - train_start_time;
#endif
  if (!should_continue) {
...
bool GBDT::EvalAndCheckEarlyStopping() {
  ...
  // print message for metric		打印评估消息（当提前终止时才有具体消息）
...
/*! \brief Get training scores result. 获取训练分数
...
/*
	预测贡献：将构造的所有树模型（迭代数*每个迭代阶段构造的树模型数）的预测结果顺序存放到output中
*/
void GBDT::PredictContrib(const double* features, double* output, const PredictionEarlyStopInstance* early_stop) const {
  ...
  for (int i = 0; i < num_iteration_for_pred_; ++i) {
    // predict all the trees for one iteration 使用当前迭代的所有树进行预测(多分类任务的一个迭代阶段有会构造多颗决策树)
    for (int k = 0; k < num_tree_per_iteration_; ++k) {
      models_[i * num_tree_per_iteration_ + k]->PredictContrib(features, num_features, output + k*(num_features + 1));
    }
    // check early stopping				检查是否提前终止
