...
/*!
	\brief 特征重要性,累计到num_iteration的特征重要性之和
*/
std::vector<double> GBDT::FeatureImportance(int num_iteration, int importance_type) const {
  ...
  if (importance_type == 0) {
    for (int iter = 0; iter < num_used_model; ++iter) {
      for (int split_idx = 0; split_idx < models_[iter]->num_leaves() - 1; ++split_idx) {
        if (models_[iter]->split_gain(split_idx) > 0) {
          feature_importances[models_[iter]->split_feature(split_idx)] += 1.0;			//被划分的次数
        }
      }
    }
    ...
    for (int iter = 0; iter < num_used_model; ++iter) {
      for (int split_idx = 0; split_idx < models_[iter]->num_leaves() - 1; ++split_idx) {
        if (models_[iter]->split_gain(split_idx) > 0) {
          feature_importances[models_[iter]->split_feature(split_idx)] += models_[iter]->split_gain(split_idx);		//划分后的信息增益
        }
      }
    }
  } else {
...
/*!
\brief 特征重要性,某个迭代的特征重要性
*/
std::vector<double> GBDT::NewFeatureImportance(int cur_iteration, int importance_type) const {

	int num_used_model = static_cast<int>(models_.size());
	if (cur_iteration > 0) {
		cur_iteration += 0;
		num_used_model = std::min(cur_iteration * num_tree_per_iteration_, num_used_model);
	}

	std::vector<double> feature_importances(max_feature_idx_ + 1, 0.0);
	int start_idx = cur_iteration - num_tree_per_iteration_;
	start_idx = start_idx < 0 ? 0 : start_idx;
	if (importance_type == 0) {
		for (int iter = start_idx; iter < num_used_model; ++iter) {
			for (int split_idx = 0; split_idx < models_[iter]->num_leaves() - 1; ++split_idx) {
				if (models_[iter]->split_gain(split_idx) > 0) {
					feature_importances[models_[iter]->split_feature(split_idx)] += 1.0;			//被划分的次数
				}
		...
		for (int iter = start_idx; iter < num_used_model; ++iter) {
			for (int split_idx = start_idx; split_idx < models_[iter]->num_leaves() - 1; ++split_idx) {
				if (models_[iter]->split_gain(split_idx) > 0) {
					feature_importances[models_[iter]->split_feature(split_idx)] += models_[iter]->split_gain(split_idx);		//划分后的信息增益
				}
			}
