...
/*
	对当前迭代进行预测。
*/
void GBDT::PredictCurIter(const double* features, double* output, const int curIter) const {
	int early_stop_round_counter = 0;
	// set zero
	std::memset(output, 0, sizeof(double) * num_tree_per_iteration_);
	// predict all the trees for one iteration
	for (int k = 0; k < num_tree_per_iteration_; ++k) {
		output[k] += models_[curIter * num_tree_per_iteration_ + k]->Predict(features);
	}
}
