...
void Application::LoadData() {
  ...
  // create training metric		创建训练评度量标准
  bool hasBinaryError = false;
  if (config_.boosting_config.is_provide_training_metric) {
    for (auto metric_type : config_.metric_types) {
      auto metric = std::unique_ptr<Metric>(Metric::CreateMetric(metric_type, config_.metric_config));
      if (metric == nullptr) { continue; }
	  if (metric_type == std::string("binary_error")) {
		  hasBinaryError = true;
	  }
  ...
  if (config_.boosting_type == std::string("gbdt_sampling")) {
	  if (config_.objective_type == std::string("binary")) {
		  ////////////////// add by zhouyu：当为二分类任务时，为每个迭代阶段进行模型评价提供基础
		  if (!hasBinaryError) {
			  auto metric = std::unique_ptr<Metric>(Metric::CreateMetric("binary_error", config_.metric_config));
			  metric->Init(train_data_->metadata(), train_data_->num_data());
			  train_metric_.push_back(std::move(metric));
		  }
		  //////////////////
	  }else {
		  Log::Fatal("gbdt_sampling 只能应用在二分类任务中");
	  }
  }
...
void Application::Train() {
  ...
  Log::Info("%f seconds elapsed, finished train", std::chrono::duration<double,
	  std::milli>(end_time - start_time) * 1e-3);
  boosting_->SaveModelToFile(-1, config_.io_config.output_model.c_str());
  // convert model to if-else statement code			将模型转换为if-else语句code
