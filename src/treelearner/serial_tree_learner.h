...
public:
  ...
  //////////
  void FeatureSamplingByImportance(const std::vector<double> feature_importance) override;

  void SetFeatureSampledIndices(const std::vector<int> feature_sampled_indices) override;

  int UsedFeatureCount() override;

  void SetFeatureSamplingByImportance(const bool feature_sampling_by_importance) override;

  std::vector<int> GetValidFeatureIndices() override;
  ////////
...
protected:
  ...
  */
  ...
  /*! \brief gradients of current iteration 当前迭代的一阶导数*/
  const score_t* gradients_;
  /*! \brief hessians of current iteration 当前迭代的二阶导数*/
  const score_t* hessians_;
  /*! \brief training data partition on leaves */
  std::unique_ptr<DataPartition> data_partition_;
  /*! \brief used for generate used features 用来生成所使用的特征 */
  Random random_;
  ...
  /*! \brief stores best thresholds for all feature for smaller leaf 对更小的叶子，存储所有特征的最佳阈值*/
  std::unique_ptr<LeafSplits> smaller_leaf_splits_;
  /*! \brief stores best thresholds for all feature for larger leaf 对于更大的叶子，存储所有特征的最佳阈值*/
  std::unique_ptr<LeafSplits> larger_leaf_splits_;
  std::vector<int> valid_feature_indices_;

  /*! \brief 存储根据重要性进行特征抽样的成员 */
  bool feature_sampling_by_importance_ = false;
  std::vector<double> feature_importance_;
  std::vector<int> feature_sampled_indices_;
  ...
  /*! \brief Store ordered bin 存储已排序的bin*/
