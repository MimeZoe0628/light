...
TreeLearner* TreeLearner::CreateTreeLearner(const std::string& learner_type, const std::string& device_type, const TreeConfig* tree_config) {
  ...
  return nullptr;
	...
	void TreeLearner::SetFeatureSamplingByImportance(const bool feature_sampling_by_importance)
	{
		return;
	}
	void TreeLearner::FeatureSamplingByImportance(const std::vector<double> feature_importance){
	
	}
	
	void TreeLearner::SetFeatureSampledIndices(std::vector<int> feature_sampled_indices) {
		return;
	}
	int TreeLearner::UsedFeatureCount() {
		return 0;
	}
	std::vector<int> TreeLearner::GetValidFeatureIndices() {
