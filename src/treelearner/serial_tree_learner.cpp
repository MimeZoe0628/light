...
std::chrono::duration<double, std::milli> feature_sampling_time;
...
void SerialTreeLearner::Init(const Dataset* train_data, bool is_constant_hessian) {
  ...
  // at least need 2 leaves 至少需要两个叶子
  ...
  // get ordered bin 获取已经排序的bin
  train_data_->CreateOrderedBins(&ordered_bins_);

  // check existing for ordered bin		检查是否存在已排序的bin
...
Tree* SerialTreeLearner::Train(const score_t* gradients, const score_t *hessians, bool is_constant_hessian) {
  ...
  #endif
  // some initial works before training 训练前的一些初始化工作
  BeforeTrain();
...
void SerialTreeLearner::SetFeatureSamplingByImportance(const bool feature_sampling_by_importance)
{
	feature_sampling_by_importance_ = feature_sampling_by_importance;
}
void SerialTreeLearner::FeatureSamplingByImportance(const std::vector<double> feature_importance)
{
	if (tree_config_->feature_fraction >= 1) return;

	feature_sampling_by_importance_ = true;
	...
	// 如果是稀疏数据，转换特征重要性为有效的特征重要性
	if (valid_feature_indices_.size() != feature_importance.size()) {
		for (int i = 0; i < valid_feature_indices_.size(); i++) {
			int valid_index = valid_feature_indices_.at(i);
			valid_feature_importance.push_back(feature_importance.at(valid_index));
		}
	}
	else {
	...
	for (int i = 0; i < valid_feature_importance.size(); i++)
	{
		int weight = valid_feature_importance.at(i);
		weights_int.push_back(weight);
	}
	M_max = *max_element(weights_int.begin(), weights_int.end());
	srand((unsigned)time(NULL));
	std::vector<int> sampled_indices;				
	for (int i = 0; i < used_feature_cnt; i++)
	{
		while (true) {
			int m = rand() % (M_max + 1);
			int n = rand() % valid_feature_indices_.size();
			if (weights_int.at(n) >= m)
			{
				if (std::find(sampled_indices.begin(), sampled_indices.end(), n) == sampled_indices.end()) {
					sampled_indices.push_back(n);
					break;
				}
			}
		}
	}
	feature_sampled_indices_ = sampled_indices;
}
int SerialTreeLearner::UsedFeatureCount() {
	return static_cast<int>(valid_feature_indices_.size()*tree_config_->feature_fraction);
}


void SerialTreeLearner::SetFeatureSampledIndices(const std::vector<int> feature_sampled_indices) {
	feature_sampled_indices_ = feature_sampled_indices;
}

std::vector<int> SerialTreeLearner::GetValidFeatureIndices()
{
	return valid_feature_indices_;
}
void SerialTreeLearner::BeforeTrain() {
  ...
  if (tree_config_->feature_fraction < 1) {
	  ...
	  std::vector<int> sampled_indices;
	  //  获取当前树使用的特征，返回特征索引-----------modified by zoe
	  if (feature_sampling_by_importance_) {
		  sampled_indices = feature_sampled_indices_;
		  //Log::Warning("feature sampling by importance: %d", used_feature_cnt);
	  }
	  else {
#ifdef TIMETAG
		  auto start_time = std::chrono::steady_clock::now();
#endif
		  sampled_indices = random_.Sample(static_cast<int>(valid_feature_indices_.size()), used_feature_cnt);
#ifdef TIMETAG
		  feature_sampling_time += std::chrono::steady_clock::now() - start_time;
#endif
		  feature_sampled_indices_ = sampled_indices;
	  }
...
#pragma omp parallel for schedule(static, 512) if (omp_loop_size >= 1024)
	  for (int i = 0; i < omp_loop_size; ++i) {
		  ...
		  is_feature_used_[inner_feature_index] = 1;		//使用抽样获得到的索引值的特征
	  }
  }
  else {	// 使用所有特征
