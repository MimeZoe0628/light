...
public:
  ...
  *			基于正则化的sum_gradients和sum_hessians计算一个叶子的输出
			该最佳值w*只依赖于损失函数的一阶导数和二阶导数
  * \param sum_gradients
  * \param sum_hessians
  * \return leaf output
  */
...
private:
  ...
  void FindBestThresholdSequence(double sum_gradient, double sum_hessian, data_size_t num_data, double min_gain_shift,
    ...
    if (dir == -1) {
      ...
      // from right to left, and we don't need data in bin0.	从右到左，不需要bin0中的数据
      for (; t >= t_end; --t) {

        // need to skip default bin	跳过默认的bin
        ...
        // mark to is splittable	将其标记为可划分的
        is_splittable_ = true;
        // better split point
        if (current_gain > best_gain) {
          best_left_count = left_count;
      ...
      for (; t <= t_end; ++t) {
        ...
        double current_gain = GetLeafSplitGain(sum_left_gradient, sum_left_hessian,
          ...
          + GetLeafSplitGain(sum_right_gradient, sum_right_hessian,
                             meta_->tree_config->lambda_l1, meta_->tree_config->lambda_l2);
        // gain with split is worse than without split		划分增益比不划分时糟糕
        if (current_gain <= min_gain_shift) continue;

        // mark to is splittable	当未通过上述不可划分的检查，则将当前节点标记为可划分的
        is_splittable_ = true;
        // better split point		如果当前划分成为最佳划分，保存到相应变量下
        if (current_gain > best_gain) {
          best_left_count = left_count;
          best_sum_left_gradient = sum_left_gradient;
          best_sum_left_hessian = sum_left_hessian;
